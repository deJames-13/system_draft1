<!-- Top Bar -->
<div class="border my-2 mb-8 md:border-0">
    <div class="container flex flex-row-reverse items-center space-x-8 mx-auto">
        <div class="py-4 flex space-x-4 mx-2 md:w-1/2">
            <div class="container rounded-full bg-primary30 border-b-2 border-accent p-2 px-2 hover:border-2">
                <input id="searchInput" type="text" class="container bg-primary30 px-4 text-lg focus:outline-none" />
            </div>
            <button onclick="searchFunction()" class="rounded-full px-4 md:border md:border-primary hover:text-secondary">
                <i class="fa fa-search md:hidden"></i>
                <span class="hidden text-lg md:block"> Search </span>
            </button>
        </div>
        <!-- <div class="p-4 md:flex md:items-center md:space-x-4 hover:text-secondary">
            <span class="hidden md:block">Filters</span>
            <i class="fa fa-filter"></i>
        </div> -->
    </div>
</div>